    ...
    const fetchData = async () => {
      try {
        ...
        const formattedData: ChartData[] = response.data.map((item) => {
          ...
          return {
            date: item.Date,
            callChange: item.Pro_Call_OI,
            putChange: item.Pro_Put_OI,
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] space-y-4">
        ...
        <p className="text-gray-600">Loading Professional Index Option OI data...</p>
      </div>
    );
  }
    ...
    <div className="w-full max-w-6xl mx-auto p-4">
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-indigo-700 to-blue-600 p-6 text-white">
          <h1 className="text-2xl font-bold mb-1">Professional OI in Index Options</h1>
          <p className="text-blue-100">
            Professional traders' Index Options (Call & Put) OI vs NIFTY 50 performance
          </p>
        </div>
              ...
              >
                ...
                <Tooltip
                  ...
                  labelFormatter={(value) => (
                    <div className="font-semibold text-gray-700">
                      {formatDate(filteredData.find(d => d.day === value)?.date || value)}
                    </div>
                  )}
                ...
                >
                  {filteredData.map((entry, index) => (
                    <Cell
                      key={`put-${index}`}
                      fill={entry.putChange < 0 ? "#f59e42" : "#facc15"}
                    />
                  ))}
                </Bar>
                <Area
        ...
        <div className="bg-gray-50 px-6 py-3 text-center text-sm text-gray-500 border-t border-gray-200">
          <p>
            ...
            <span className="text-green-600 mx-2">Green: Call Net Buying</span> |
            <span className="text-red-500 mx-2">Red: Call Net Selling</span> |
            <span className="text-yellow-500 mx-2">Yellow: Put Net Buying</span> |
            <span className="text-orange-500">Orange: Put Net Selling</span>
          </p>
        </div>
      </div>
    </div>
