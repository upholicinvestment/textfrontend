...
const Dii_OI_Index_Fut: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const response = await axios.get<FIIData[]>("https://api.upholictech.com/api/OIDII_Index_Fut/data");
        
        const formattedData = response.data.map((item) => {
          const dateObj = new Date(item.Date);
          return {
            date: item.Date,
            callChange: item.DII_Futures_OI,
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] space-y-4">
        ...
        <p className="text-gray-600">Loading DII Open Interest data...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] p-6 bg-red-50 rounded-lg">
        ...
        <button 
          ...
          className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-blue-700 transition-colors"
        >
          Retry
        </button>
      </div>
    ...
    <div className="w-full max-w-6xl mx-auto p-4">
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-indigo-700 to-blue-600 p-6 text-white">
          <h1 className="text-2xl font-bold mb-2">DII Open Interest in Index Futures</h1>
          <p className="text-blue-100">
            Domestic Institutional Investors' Open Interest in Index Futures vs NIFTY 50
          </p>
        </div>
...
export default Dii_OI_Index_Fut;
