...
const ROWS = 5; // show 5 rows everywhere
...
// role: null while loading -> "admin" | "superuser" | "customer"
...
function cls(theme: "dark" | "light", a: string, b: string) {
  return theme === "dark" ? a : b;
}
...
export default function AdminDashboard() {
  ...
  // theme (no external config; dark default)
  const [theme, setTheme] = useState<"dark" | "light">("dark");

  // redirect only after role resolved
  useEffect(() => {
    if (role === null) return;
    if (!isAdmin) navigate("/");
  ...
  useEffect(() => {
    ...
    setRenewalsPage(1); // reset paging when days change
    (async () => {
      try {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${API_BASE}/careers/admin/applications?${params.toString()}`, {
          headers: hdrs,
        });
        const data = await res.json();
        setApps(data);
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({
      page: String(resumesPage),
      pageSize: String(ROWS), // 5 rows
      ...(resumesQ ? { q: resumesQ } : {}),
    });
    (async () => {
      try {
        const res = await fetch(`${API_BASE}/careers/admin/resumes?${params.toString()}`, {
          headers: hdrs,
        });
        const data = await res.json();
        setResumes(data);
  ...
  // View / Download resume (auth via blob)
  async function fetchResumeBlob(collection: "career_applications" | "career_resume", fileId: string) {
    const res = await fetch(`${API_BASE}/careers/admin/resume/${collection}/${fileId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  async function handleViewResume(
    collection: "career_applications" | "career_resume",
    fileId?: string
  ) {
    if (!fileId) return;
    try {
  ...
  if (role === null) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="rounded-xl border border-white/10 p-6 bg-[#0e102b]/80 text-white/70">
  ...
  const usersHasMore =
    (usersResp?.page || 1) * (usersResp?.pageSize || ROWS) < (usersResp?.total || 0);
  const usersHasPrev = (usersResp?.page || 1) > 1;

  const appsHasMore =
    (apps?.page || 1) * (apps?.pageSize || ROWS) < (apps?.total || 0);
  const appsHasPrev = (apps?.page || 1) > 1;

  const resumesHasMore =
    (resumes?.page || 1) * (resumes?.pageSize || ROWS) < (resumes?.total || 0);
  ...
  return (
    <div className={`${bg} min-h-[100vh]`}>
      ...
      <div className="max-w-7xl mx-auto px-4 pt-6 pb-2 relative">
        {/* Right controls in one row to prevent collision */}
        <div className="absolute right-4 top-6 flex items-center gap-2">
          <button
            onClick={() => setTheme((t) => (t === "dark" ? "light" : "dark"))}
            className={`rounded-full p-2 border ${border} ${hoverPanel} transition`}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pr-24">
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-lg ${subtleBadgeBg} ${border} border ${subtleBadgeText}`}>
              <FiShield size={18} />
            </div>
        ...
        <p className={`mt-2 ${subtext}`}>Overview, users, renewals, careers.</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          {[
            ...
            <div key={k.label} className={`rounded-xl border ${border} ${panel} p-5`}>
              <div className="flex items-center justify-between">
                <div className={`${text}/80`}>{k.label}</div>
                {k.icon}
              </div>
        ...
        <div className={`rounded-xl border ${border} ${panel}`}>
          <div
  className={`px-5 py-4 border-b ${divider}
...
>
  ...
  {/* Search (mobile: full width; desktop: fixed, wider) */}
  <div className="w-full sm:w-auto">
    <div
      className={`flex items-center gap-2 ${inputBg} rounded-lg px-3 py-2 border ${border}
                  w-full sm:w-[22rem] md:w-[28rem] lg:w-[34rem] max-w-full`}
    >
      ...
      <input
        ...
        className={`w-full min-w-0 bg-transparent text-sm outline-none ${text} placeholder:${faint}`}
      />
    </div>
  </div>
</div>
          ...
          <div className="p-4 overflow-x-auto">
                      ...
                      <td className="py-3 px-3">
                        {(u.purchases || []).length === 0 ? (
                          ...
                          <ul className="space-y-1">
                            {u.purchases!.map((p) => (
                              <li key={p._id} className={`${text}/80`}>
                                <span className="font-medium">{p.productName || p.productKey}</span>{" "}
            ...
            <div className="flex items-center justify-end gap-2 mt-4">
              <button
                ...
                className={`px-3 py-1.5 rounded border text-sm ${border} ${usersHasPrev ? hoverPanel : "opacity-40 cursor-not-allowed"} ${text}/80`}
              >
                ← Prev
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded border text-sm ${border} ${usersHasMore ? hoverPanel : "opacity-40 cursor-not-allowed"} ${text}/80`}
              >
                Next →
              </button>
            </div>
          ...
          <div className={`px-5 py-4 border-b ${divider} flex items-center justify-between`}>
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <input
                ...
                className={`w-20 ${inputBg} border ${border} rounded-lg px-3 py-1.5 text-sm ${text} outline-none`}
              />
            </div>
          </div>
          <div className="p-5 overflow-x-auto">
            {loadingRenewals ? (
              ...
              <>
                    ...
                    {visibleRenewals.map((r) => (
                      <tr key={r._id} className={`border-b ${rowBorder}`}>
                        ...
                        <td className="py-3 px-3 text-center">
                          <div className={text}>{fmtDate(r.endsAt)}</div>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                ...
                <div className="flex items-center justify-end gap-2 mt-4">
                  <button
                    disabled={!hasPrevRenewals}
                    onClick={() => setRenewalsPage((p) => Math.max(1, p - 1))}
                    
                    className={`px-3 py-1.5 rounded border text-sm ${border} ${hasPrevRenewals ? hoverPanel : "opacity-40 cursor-not-allowed"} ${text}/80`}
                  >
                    ← Prev
                  </button>
                  <button
                    disabled={!moreRenewals}
                    onClick={() => setRenewalsPage((p) => p + 1)}
                    className={`px-3 py-1.5 rounded border text-sm ${border} ${moreRenewals ? hoverPanel : "opacity-40 cursor-not-allowed"} ${text}/80`}
                  >
                    Next →
                  </button>
                </div>
        ...
        <div className={`rounded-xl border ${border} ${panel} mt-8 overflow-hidden`}>
          ...
          <div className={`px-5 py-4 border-b ${divider} flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4`}>
            ...
            {/* Tabs */}
            <div className="flex items-center gap-2 bg-white/5 rounded-xl p-1 border ${border} self-start">
              <button
          ...
          <div className={`px-5 py-4 border-b ${divider}`}>
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
              ...
              <div className={`flex items-center gap-3 ${subtext} text-sm bg-black/10 rounded-lg px-3 py-2 border ${border} self-stretch sm:self-auto`}>
                  ...
                  {loadingApps ? (
                    ...
                    <tr key={a._id} className={`border-b ${rowBorder} group hover:bg-white/5 transition-colors`}>
                      ...
                      <td className={`py-4 px-4 border-r ${divider} text-center`}>
                        <span className={`${text}/80 font-mono text-xs bg-black/20 rounded px-2 py-1`}>
                          {a.jobId || "-"}
                        </span>
                      </td>
                      ...
                      {/* Resume Actions Column */}
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2 justify-center">
                          <button
                            className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border ${border} text-xs font-medium transition-all duration-200 ${hoverPanel} hover:scale-105 active:scale-95 ${text}/80`}
                            onClick={() => handleViewResume("career_applications", a.resumeFileId)}
                            title="View Resume"
                          >
                          ...
                          <button
                            className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border ${border} text-xs font-medium transition-all duration-200 ${hoverPanel} hover:scale-105 active:scale-95 ${text}/80`}
                            onClick={() => handleDownloadResume("career_applications", a.resumeFileId, a.resumeFilename)}
              ...
              {(apps?.items?.length || 0) > 0 && (
                <div className="flex items-center justify-between gap-4 mt-6 pt-4 border-t ${divider}">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      disabled={!appsHasPrev}
                      onClick={() => setAppsPage((p) => Math.max(1, p - 1))}
                      className={`px-4 py-2 rounded-lg border text-sm font-medium transition-all duration-200 ${
                        appsHasPrev 
                          ? `${border} ${hoverPanel} ${text}/80 hover:scale-105` 
                    ...
                    <tr key={r._id} className={`border-b ${rowBorder} group hover:bg-white/5 transition-colors`}>
                      ...
                      <td className={`py-4 px-4 border-r ${divider}`}>
                        <div className="flex flex-col gap-2 items-center">
                          {r.email && (
                            <div className="flex items-center gap-2">
                              <FiMail className={faint} size={12} />
                              <span className={`${text}/80 text-xs`}>{r.email}</span>
                            </div>
                          )}
                          {r.phone && (
                            <div className="flex items-center gap-2">
                              <FiPhone className={faint} size={12} />
                              <span className={`${text}/80 text-xs`}>{r.phone}</span>
                            </div>
                          )}
                          {!r.email && !r.phone && (
                            <span className={`${subtext} text-xs`}>No contact info</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2 justify-center">
                          <button
                            className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border ${border} text-xs font-medium transition-all duration-200 ${hoverPanel} hover:scale-105 active:scale-95 ${text}/80`}
                            onClick={() => handleViewResume("career_resume", r.resumeFileId)}
                            title="View Resume"
                          >
                            <FiExternalLink size={14} />
                            View
                          ...
                          <button
                            className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border ${border} text-xs font-medium transition-all duration-200 ${hoverPanel} hover:scale-105 active:scale-95 ${text}/80`}
                            onClick={() => handleDownloadResume("career_resume", r.resumeFileId, r.resumeFilename)}
                            title="Download Resume"
                          >
                            <FiDownload size={14} />
                            Download
              ...
              {(resumes?.items?.length || 0) > 0 && (
                <div className="flex items-center justify-between gap-4 mt-6 pt-4 border-t ${divider}">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      disabled={!resumesHasMore}
                      onClick={() => setResumesPage((p) => p + 1)}
                      className={`px-4 py-2 rounded-lg border text-sm font-medium transition-all duration-200 ${
                        resumesHasMore 
                          ? `${border} ${hoverPanel} ${text}/80 hover:scale-105` 
                          : "opacity-40 cursor-not-allowed"
