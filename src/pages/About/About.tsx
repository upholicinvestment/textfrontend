...
const heroData: Record<TabKey, { title: string; highlight: string; description: string; image: string }> = {
  about: {
...
export default function About() {
  ...
  return (
    <>
      <Navbar />
      <div ref={containerRef} className="min-h-screen w-full bg-neutral-950 overflow-hidden relative">
        {/* Hero Section */}
        <section className="relative h-screen overflow-hidden">
          {/* Animated background (SINGLE CHILD inside AnimatePresence) */}
          ...
          <div className="relative h-full flex flex-col justify-center items-center text-center px-6">
            <AnimatePresence mode="wait" initial={false}>
              ...
              >
                <h1 className="text-5xl md:text-7xl font-light text-white mb-6 tracking-tight">
                  {heroData[tab].title}{" "}
                  <span className="font-medium text-white">{heroData[tab].highlight}</span>
                </h1>
            ...
            <div className="flex space-x-0 bg-neutral-900 rounded-full p-1 border border-neutral-800">
              {(Object.keys(heroData) as TabKey[]).map((key) => (
                <button
                  key={`tabbtn-${key}`}
                  onClick={() => setTab(key)}
        ...
        {/* Content Sections (SINGLE CHILD inside AnimatePresence) */}
