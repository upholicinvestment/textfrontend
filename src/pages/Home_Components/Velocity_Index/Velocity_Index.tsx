...
.vi-chip { background: var(--vi-surface); border: 1px solid var(--vi-brd); color: var(--vi-fg); border-radius: 8px; }
.vi-select, .vi-input {
  border: 1.5px solid var(--vi-brd); background: var(--vi-surface); color: var(--vi-fg);
  border-radius: 10px; padding: 10px 14px; height: 42px; font-size: 14px; font-weight: 500;
  transition: border-color .2s ease, box-shadow .2s ease;
}
.vi-select:focus, .vi-input:focus { outline: none; border-color: var(--vi-accent); box-shadow: 0 0 0 3px rgba(99,102,241,.15); }
.vi-select { appearance: none; background-image: url("data:image/svg+xml,%3Csvg width='12' height='8' viewBox='0 0 12 8' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1.5L6 6.5L11 1.5' stroke='%2394a3b8' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E"); background-repeat: no-repeat; background-position: right 12px center; padding-right: 36px; }
.vi-label { font-size: 12px; font-weight: 600; color: var(--vi-muted); margin-bottom: 6px; letter-spacing: .025em; text-transform: uppercase; }
.no-scrollbar::-webkit-scrollbar{ display:none; } .no-scrollbar{ -ms-overflow-style:none; scrollbar-width:none; }
.vi-thead { position: sticky; top: 0; z-index: 1; background: var(--vi-surface); }
.vi-thead th { background: linear-gradient(180deg, var(--vi-head-grad-1), var(--vi-head-grad-2)), var(--vi-surface); color: var(--vi-fg); font-weight: 700; letter-spacing: .05em; text-transform: uppercase; font-size: 11px; border-bottom: 1px solid var(--vi-brd); }
.vi-vlines th + th, .vi-vlines td + td { border-left: 1px solid var(--vi-brd); } .vi-vlines tbody td { border-bottom: 1px solid var(--vi-brd); } .vi-vlines tbody tr:last-child td { border-bottom: none; }
.vi-table-row { transition: background-color .12s ease; } .vi-table-row:hover { background-color: var(--vi-row-hover); }
.vi-iconbtn { width: 42px; height: 42px; border-radius: 10px; display: grid; place-items: center; border: 1.5px solid var(--vi-brd); background: var(--vi-surface); color: var(--vi-fg); cursor: pointer; transition: transform .15s ease, box-shadow .15s ease, border-color .15s; }
.vi-iconbtn:hover:not(:disabled) { border-color: var(--vi-accent); box-shadow: 0 0 0 3px rgba(99,102,241,.12); transform: translateY(-1px); }
