...
import {
  Calendar, Edit3, Save, Plus, X, TrendingUp, Target, Shield, BarChart3, Award, Info,
   Brain, Star, AlertCircle, CheckCircle, XCircle, Trash2
} from "lucide-react";

// API endpoints
const API_BASE = "https://api.upholictech.com/api/daily-journal";
const INSTRUMENT_API = "https://api.upholictech.com/api/instruments/search";
const UNDERLYINGS_API = "https://api.upholictech.com/api/instruments/underlyings";
const EXPIRIES_API = "https://api.upholictech.com/api/instruments/expiries"; // NEW
...
const strategies = [
  "Breakout", "Pullback", "Momentum", "Reversal", "Trend Following",
  "Gap Up/Down", "Mean Reversion", "Option Selling", "Scalping",
  "VWAP", "EMA Crossover",
];
const exchanges = ["NSE", "BSE"];
const instrumentTypes = [
  "EQUITY", "FUTIDX", "FUTSTK", "OPTIDX", "OPTSTK", "OPTFUT", "INDEX", "FUTCUR", "FUTCOM"
];
...
function getBadgeObj(executionPercent: number) {
  if (executionPercent >= 90)
    return { badge: "MASTER", color: "from-emerald-500 to-emerald-600", textColor: "text-emerald-50", bgColor: "bg-emerald-900/20" };
  if (executionPercent >= 75)
    return { badge: "EXPERT", color: "from-blue-500 to-blue-600", textColor: "text-blue-50", bgColor: "bg-blue-900/20" };
  if (executionPercent >= 60)
    return { badge: "SKILLED", color: "from-amber-500 to-amber-600", textColor: "text-amber-50", bgColor: "bg-amber-900/20" };
  return { badge: "LEARNING", color: "from-slate-500 to-slate-600", textColor: "text-slate-50", bgColor: "bg-slate-800/20" };
}
...
const TradeDetailModal = ({
  open, onClose, trade, isExtra
}: { open: boolean; onClose: () => void; trade: any; isExtra?: boolean }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-slate-900 rounded-2xl p-6 max-w-lg w-full mx-4 border border-slate-700 shadow-2xl">
        ...
        <div className="mb-3">
          <div className="text-xl font-bold text-white mb-1">{trade.symbol}</div>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          {trade && typeof trade === "object" && Object.entries(trade).map(([k, v], i) =>
            v !== undefined && v !== null && k !== 'symbol' && k !== 'tradeType' && (
...
export default function DailyJournal() {
  ...
  const [plannedTrades, setPlannedTrades] = useState<PlannedTrade[]>([{ ...defaultTrade }]);
  const [planNotes, setPlanNotes] = useState("");
  const [editMode, setEditMode] = useState(false);
  const [saving, setSaving] = useState(false);
  const [comparison, setComparison] = useState<Comparison | null>(null);
  const [executedStatus, setExecutedStatus] = useState<"ok" | "no-executions">("ok");
  ...
  const calculatePnL = (trade: PlannedTrade) => {
    ...
    const priceDiff = trade.tradeType === "BUY" ? trade.target - trade.entry : trade.entry - trade.target;
    return priceDiff * trade.quantity;
  };

  const approxPL = plannedTrades.reduce((sum, t) => sum + calculatePnL(t), 0);

  const updateTradeWithQuantity = (trade: PlannedTrade) => ({ ...trade, quantity: calculateQuantity(trade) });

  // Fetch plan & comparison for selected date
  useEffect(() => {
    const key = formatDateKey(selectedDate);
    axios.get(`${API_BASE}/plan?date=${key}`).then(res => {
      if (res.data && res.data.date) {
        setPlanNotes(res.data.planNotes ?? "");
        const loaded = (Array.isArray(res.data.plannedTrades) && res.data.plannedTrades.length
        ...
        setActiveEditIndex(firstEmpty !== -1 ? firstEmpty : Math.max(loaded.length - 1, 0));

        setPsych({
          confidenceLevel: res.data.confidenceLevel ?? defaultPsych.confidenceLevel,
        ...
        const t = loaded[firstEmpty !== -1 ? firstEmpty : Math.max(loaded.length - 1, 0)];
        if (t?.instrumentType === "OPTIDX" && (t.underlyingSymbol || "").trim()) {
          loadUnderlyings(t.exchangeId, t.instrumentType);
          loadExpiries(t.exchangeId, t.instrumentType, t.optionType, t.underlyingSymbol);
        } else {
          setExpiryOptions([]);
        }
      } else {
  ...
  const loadUnderlyings = (exch?: string, instr?: string) => {
    const ex = (exch || plannedTrades[activeEditIndex]?.exchangeId || "").trim();
    const it = (instr || plannedTrades[activeEditIndex]?.instrumentType || "").trim();
    if (ex && it === "OPTIDX") {
      axios
        .get(`${UNDERLYINGS_API}?exchange=${encodeURIComponent(ex)}&instrument=${encodeURIComponent(it)}`)
  ...
  const loadExpiries = (
    ...
    const ex = (exchangeId || plannedTrades[activeEditIndex]?.exchangeId || "").trim();
    const it = (instrumentType || plannedTrades[activeEditIndex]?.instrumentType || "").trim();
    const ot = (optionType || plannedTrades[activeEditIndex]?.optionType || "").trim();
    const und = (underlyingSymbol || plannedTrades[activeEditIndex]?.underlyingSymbol || "").trim();
    ...
    if (it === "OPTIDX" && ex && und) {
      ...
      if (ot) params.set("optionType", ot);
      axios
        .get(`${EXPIRIES_API}?${params.toString()}`)
  ...
  const debouncedSuggest = useRef(
    debounce((query: string, idx: number, exchangeId?: string, instrumentType?: string, optionType?: string, underlyingSymbol?: string, expiry?: string) => {
      if (!query) return setSymbolSuggestions([]);
      let url = `${INSTRUMENT_API}?query=${encodeURIComponent(query)}`;
      if (exchangeId) url += `&exchange=${encodeURIComponent(exchangeId)}`;
      if (instrumentType) url += `&instrumentType=${encodeURIComponent(instrumentType)}`;
      if (optionType) url += `&optionType=${encodeURIComponent(optionType)}`;
      if (underlyingSymbol) url += `&underlying=${encodeURIComponent(underlyingSymbol)}`;
      if (expiry) url += `&expiry=${encodeURIComponent(expiry)}`; // NEW
  ...
  // ----------- Handlers -----------
  const handleExchangeChange = (e: ChangeEvent<HTMLSelectElement>, idx: number) => {
    ...
    if (arr[idx].instrumentType === "OPTIDX") loadExpiries(e.target.value, arr[idx].instrumentType, arr[idx].optionType, arr[idx].underlyingSymbol);
    else setExpiryOptions([]);
  };

  const handleInstrumentTypeChange = (e: ChangeEvent<HTMLSelectElement>, idx: number) => {
    const arr = [...plannedTrades];
    ...
    if (e.target.value === "OPTIDX") loadExpiries(arr[idx].exchangeId, e.target.value, arr[idx].optionType, arr[idx].underlyingSymbol);
    else setExpiryOptions([]);
  };

  const handleOptionTypeChange = (e: ChangeEvent<HTMLSelectElement>, idx: number) => {
    const arr = [...plannedTrades];
    ...
    setSymbolSuggestions([]);
    setSuggestForIdx(null);
    if (arr[idx].instrumentType === "OPTIDX") loadExpiries(arr[idx].exchangeId, arr[idx].instrumentType, e.target.value, arr[idx].underlyingSymbol);
  };

  const handleUnderlyingChange = (e: ChangeEvent<HTMLSelectElement>, idx: number) => {
    const arr = [...plannedTrades];
    ...
    setSymbolSuggestions([]);
    setSuggestForIdx(null);
    if (arr[idx].instrumentType === "OPTIDX") loadExpiries(arr[idx].exchangeId, arr[idx].instrumentType, arr[idx].optionType, e.target.value);
  };

  const handleExpiryChange = (e: ChangeEvent<HTMLSelectElement>, idx: number) => {
    const arr = [...plannedTrades];
  ...
  const handleSymbolChange = (e: ChangeEvent<HTMLInputElement>, idx: number) => {
    const arr = [...plannedTrades];
  ...
  const getNextActiveAfterDelete = (deletedIdx: number, prevActive: number, newLen: number) => {
  ...
  const removeTradeRow = (idx: number) =>
    setPlannedTrades((trades) => {
      ...
      setActiveEditIndex((prev) => getNextActiveAfterDelete(idx, prev, next.length));
      return next;
    });
  ...
  const SuggestionDropdown = ({ suggestions, idx }: { suggestions: any[]; idx: number }) => (
    ...
    {suggestions.map((s, k) => (
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          {s.instrumentType && (
            <span className="bg-blue-900/40 text-blue-300 px-1.5 py-0.5 rounded text-xs">{s.instrumentType}</span>
          )}
          {s.optionType && (
            <span className="bg-purple-900/40 text-purple-300 px-1.5 py-0.5 rounded text-xs">{s.optionType}</span>
          )}
          <span className={`bg-green-900/40 text-green-300 px-1.5 py-0.5 rounded text-xs`}>
            {s.exchangeId}
          </span>
          {s.underlyingSymbol && (
            <span className="bg-slate-700/60 text-slate-200 px-1.5 py-0.5 rounded text-xs">{s.underlyingSymbol}</span>
          )}
        </div>
  ...
  const activeTrade: PlannedTrade = plannedTrades[activeEditIndex] ?? plannedTrades[0];
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  if (!editMode) {
                    const firstEmpty = plannedTrades.findIndex(t => !t.symbol);
                    setActiveEditIndex(firstEmpty !== -1 ? firstEmpty : Math.max(plannedTrades.length - 1, 0));
                  }
                }}
            ...
            {executedStatus === "no-executions" ? (
              <div className="mb-8">
                <div className="bg-slate-800/50 rounded-2xl border border-slate-700 shadow-xl px-6 py-8 flex flex-col items-center">
                  ...
                  <div className="text-2xl font-bold text-white mb-1">No Execution Data</div>
                  <div className="text-slate-400 mb-1 text-center">
                    You haven't uploaded an orderbook for this date.<br />
                    <span className="text-indigo-400">Upload your trade orderbook to analyze execution!</span>
                  </div>
                </div>
              </div>
            ) : (
                  ...
                  <div className="mb-8">
                    <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl border border-slate-700 overflow-hidden shadow-2xl">
                      <div className="p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div className="flex items-center gap-3">
                          <div className={`flex items-center gap-3 px-6 py-2.5 rounded-xl bg-gradient-to-r ${getBadgeObj(comparison.executionPercent).color} shadow-xl`}>
                            <Award className="w-5 h-5 text-white" />
                            <span className="font-bold text-white">{getBadgeObj(comparison.executionPercent).badge}</span>
                          </div>
                          <div>
                            <h3 className="text-lg font-semibold text-white mb-0.5">Execution Performance</h3>
                            <p className="text-slate-400 text-sm">
                              {comparison.matched} of {comparison.totalPlanned} planned trades executed
                            </p>
                          </div>
                        </div>
                        <div className="text-center">
                          <div className="text-4xl font-bold text-white mb-0.5">{comparison.executionPercent}%</div>
                      ...
                      <div className="bg-slate-700/30 px-6 py-3">
                        <div className="w-full bg-slate-700 rounded-full h-2.5">
                          <div
                            className={`h-2.5 rounded-full bg-gradient-to-r ${getBadgeObj(comparison.executionPercent).color} transition-all duration-1000`}
                  ...
                  <div className="grid lg:grid-cols-2 gap-6 mb-8">
                    <div className="bg-slate-800/50 border border-green-800/30 rounded-xl p-6">
                      <div className="flex items-center gap-2 mb-4">
                        <CheckCircle className="w-5 h-5 text-green-400" />
                        <h3 className="text-lg font-semibold text-green-300">Key Insights</h3>
                      </div>
                      <ul className="space-y-2">
                        {(comparison.insights?.length
                          ? comparison.insights.filter(x =>
                              !!x.trim() &&
                              !x.includes("P&L: ₹0.00") &&
                    ...
                    <div className="bg-slate-800/50 border border-red-800/30 rounded-xl p-6">
                      <div className="flex items-left gap-2 mb-4">
                        ...
                        <h3 className="text-lg font-semibold text-red-300">Areas for Improvement</h3>
                      </div>
                      <ul className="space-y-2">
                    ...
                    <div className="bg-green-950/40 border border-green-700 rounded-xl p-5 text-center flex flex-col justify-between min-h-[140px]">
                      <div>
                        <div className="flex items-center gap-2 mb-2 justify-center">
                          <CheckCircle className="w-5 h-5 text-green-400" />
                          <span className="font-semibold text-green-300">Executed as Planned</span>
                        </div>
                        <div className="text-3xl font-bold text-green-200 mb-1">{comparison.matchedTrades.length}</div>
                      </div>
                      <button
                        disabled={comparison.matchedTrades.length === 0}
                        className={`mt-3 px-3 py-1.5 rounded-lg text-green-200 font-medium transition
                          ${comparison.matchedTrades.length ? "bg-green-800/40 hover:bg-green-700/30" : "bg-green-900/30 opacity-50 cursor-not-allowed"}`}
                    ...
                    <div className="bg-yellow-950/40 border border-yellow-700 rounded-xl p-5 text-center flex flex-col justify-between minHeight-[140px]">
                      <div>
                        <div className="flex items-center gap-2 mb-2 justify-center">
                          ...
                          <span className="font-semibold text-yellow-300">Missed Opportunities</span>
                        </div>
                        <div className="text-3xl font-bold text-yellow-200 mb-1">{comparison.missedTrades.length}</div>
                      </div>
                      <button
                        disabled={comparison.missedTrades.length === 0}
                        className={`mt-3 px-3 py-1.5 rounded-lg text-yellow-200 font-medium transition
                          ${comparison.missedTrades.length ? "bg-yellow-800/40 hover:bg-yellow-700/30" : "bg-yellow-900/30 opacity-50 cursor-not-allowed"}`}
                    ...
                    <div className="bg-purple-950/40 border border-purple-700 rounded-xl p-5 text-center flex flex-col justify-between min-h-[140px]">
                      <div>
                        <div className="flex items-center gap-2 mb-2 justify-center">
                          ...
                          <span className="font-semibold text-purple-300">Unplanned Trades</span>
                        </div>
                        <div className="text-3xl font-bold text-purple-200 mb-1">{comparison.extraTrades.length}</div>
                      </div>
                      <button
                        disabled={comparison.extraTrades.length === 0}
                        className={`mt-3 px-3 py-1.5 rounded-lg text-purple-200 font-medium transition
                          ${comparison.extraTrades.length ? "bg-purple-800/40 hover:bg-purple-700/30" : "bg-purple-900/30 opacity-50 cursor-not-allowed"}`}
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
    <div className="bg-slate-900 rounded-2xl p-6 max-w-xl w-full mx-4 border border-slate-700 shadow-2xl relative">
      ...
      <h2 className="text-xl font-bold mb-4 text-white flex items-center gap-2">
        {modalType === "matched" && <CheckCircle className="w-5 h-5 text-green-400" />}
        {modalType === "missed" && <AlertCircle className="w-5 h-5 text-yellow-400" />}
        {modalType === "extra" && <Info className="w-5 h-5 text-purple-400" />}
      ...
      <div
        ...
        "
      >
        {modalType === "matched" && comparison.matchedTrades.map((t, i) => (
          <div key={i} className="bg-green-800/30 text-green-100 rounded-xl p-4">
            <div className="flex flex-wrap gap-3 items-center mb-1">
              <span className="font-semibold">{t.symbol}</span>
              {t.expiry && <span className="text-xs text-green-200">{t.expiry}</span>}
              <span className="text-xs">{t.tradeType}</span>
              <span className="ml-auto font-medium">@ ₹{t.entry}</span>
              {typeof t.PnL !== "undefined" &&
                <span className={`ml-3 px-2 py-0.5 rounded text-xs font-semibold ${t.PnL >= 0 ? "bg-green-900/60" : "bg-red-900/60"}`}>
                  P&L: ₹{t.PnL}
                </span>}
              {t.quantity && <span className="text-xs text-green-300">Qty: {t.quantity}</span>}
            </div>
          </div>
        ))}
        {modalType === "missed" && comparison.missedTrades.map((t, i) => (
          <div key={i} className="bg-yellow-800/30 text-yellow-100 rounded-xl p-4">
            <div className="flex flex-wrap gap-3 items-center mb-1">
              <span className="font-semibold">{t.symbol}</span>
              {t.expiry && <span className="text-xs text-yellow-200">{t.expiry}</span>}
              <span className="text-xs">{t.tradeType}</span>
              <span className="ml-auto font-medium">Planned @ ₹{t.entry}</span>
              {t.quantity && <span className="text-xs text-yellow-300">Qty: {t.quantity}</span>}
            </div>
            {t.reason && <div className="text-xs text-yellow-300 mt-1">{t.reason}</div>}
          </div>
        ))}
        {modalType === "extra" && comparison.groupedExtras &&
          Object.values(comparison.groupedExtras).map((trades, idx) => {
            const t = trades[0];
            return (
              <div key={idx} className="bg-purple-800/30 text-purple-100 rounded-xl p-4">
                <div className="flex flex-wrap gap-3 items-center mb-1">
                  <span className="font-semibold">{t.symbol}</span>
                  {t.expiry && <span className="text-xs text-purple-200">{t.expiry}</span>}
                  <span className="text-xs">{t.tradeType}</span>
                  <span className="ml-auto font-medium">{trades.length > 1 ? `x${trades.length}` : null}</span>
                </div>
                <div className="flex flex-wrap gap-3 mt-1 text-xs text-purple-300">
                  {trades.map((x, k) => (
                    <span key={k}>@ ₹{x.entry} (Qty: {x.quantity || 1})</span>
                  ))}
                </div>
              </div>
            );
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="text-base font-semibold text-white">Mental State Assessment</h3>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={e => setPsych(ps => ({ ...ps, confidenceLevel: Number(e.target.value) }))}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={e => setPsych(ps => ({ ...ps, stressLevel: Number(e.target.value) }))}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={e => setPsych(ps => ({ ...ps, distractions: e.target.value }))}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={e => setPsych(ps => ({ ...ps, sleepHours: Number(e.target.value) }))}
                          placeholder="Hours"
                        />
                      </div>
                    </div>
                  ...
                  {plannedTrades.some((t, i) => i !== activeEditIndex && t.symbol) && (
                    <div>
                        ...
                        <table className="w-full border-collapse">
                          ...
                          <tbody>
                            {plannedTrades.map((t, i) => {
                              if (i === activeEditIndex || !t.symbol) return null;
                              ...
                              return (
                                <tr key={i} className="border-b border-slate-700 hover:bg-slate-700/20">
                                  <td className="p-2">
                                    ...
                                    {t.expiry && <div className="text-[11px] text-slate-400">{t.expiry}</div>}
                                  </td>
                                  <td className="p-2 text-center">
                                    <span className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${
                                      t.tradeType === "BUY" ? "bg-green-900/40 text-green-300" : "bg-red-900/40 text-red-300"
                                    }`}>
                                      {t.tradeType}
                                    </span>
                                  </td>
                                  <td className="p-2 text-center text-slate-300 text-xs">{t.strategy}</td>
                                  <td className="p-2 text-center text-white font-medium">₹{t.entry || "0.00"}</td>
                                  <td className="p-2 text-center text-green-400 font-medium">₹{t.target || "0.00"}</td>
                                  <td className="p-2 text-center text-red-400 font-medium">₹{t.stopLoss || "0.00"}</td>
                                  <td className="p-2 text-center text-white">{t.lots}</td>
                                  <td className="p-2 text-center text-white">{t.quantity || "-"}</td>
                                  <td className={`p-2 text-center font-medium ${potentialPL >= 0 ? "text-green-400" : "text-red-400"}`}>
                  ...
                  <div>
                    ...
                    <div className="bg-slate-700/40 rounded-xl p-5 border border-slate-600 space-y-5">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <div>
                          <label className="block text-xs font-medium text-slate-300 mb-1.5">Exchange</label>
                          <select
                            className="w-full bg-slate-600 border border-slate-500 rounded-lg px-3 py-2.5 text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            value={activeTrade.exchangeId}
                            onChange={e => handleExchangeChange(e as any, activeEditIndex)}
                          >
                        ...
                        <div>
                          <label className="block text-xs font-medium text-slate-300 mb-1.5">Instrument Type</label>
                          <select
                            className="w-full bg-slate-600 border border-slate-500 rounded-lg px-3 py-2.5 text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            value={activeTrade.instrumentType}
                            onChange={e => handleInstrumentTypeChange(e as any, activeEditIndex)}
                          >
                      ...
                      <div className={`grid grid-cols-1 ${activeTrade.instrumentType === "OPTIDX" ? "md:grid-cols-4" : "md:grid-cols-3"} gap-3`}>
                        {["OPTIDX", "OPTSTK", "OPTFUT"].includes(activeTrade.instrumentType || "") && (
                          <div>
                            <label className="block text-xs font-medium text-slate-300 mb-1.5">Option Type</label>
                            <select
                              className="w-full bg-slate-600 border border-slate-500 rounded-lg px-3 py-2.5 text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                              value={activeTrade.optionType}
                              onChange={e => handleOptionTypeChange(e as any, activeEditIndex)}
                            >
                        ...
                        {activeTrade.instrumentType === "OPTIDX" && (
                          <div>
                            <label className="block text-xs font-medium text-slate-300 mb-1.5">Index</label>
                            <select
                              className="w-full bg-slate-600 border border-slate-500 rounded-lg px-3 py-2.5 text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                              value={activeTrade.underlyingSymbol || ""}
                              onChange={e => handleUnderlyingChange(e as any, activeEditIndex)}
...
{activeTrade.instrumentType === "OPTIDX" && (
  ...
  <div className="relative">
    <select
      ...
      onChange={e => handleExpiryChange(e as any, activeEditIndex)}
      onFocus={() => loadExpiries()}
    >
      <option value="">Select Expiry</option>
      {expiryOptions.map(d => (
        <option key={d} value={d} className="bg-slate-900 hover:bg-slate-800 p-2">{d}</option>
      ))}
    </select>
    ...
    >
      <path d="M5.23 7.21a.75.75 0 0 1 1.06.02L10 10.94l3.71-3.71a.75.75 0 1 1 1.06 1.06l-4.24 4.24a.75.75 0 0 1-1.06 0L5.21 8.29a.75.75 0 0 1 .02-1.08z"/>
                        ...
                        <div className="relative">
                          ...
                          <input
                            ref={el => { symbolInputRefs.current[activeEditIndex] = el; }}
                            ...
                            autoComplete="off"
                            onChange={e => handleSymbolChange(e as any, activeEditIndex)}
                            onFocus={() =>
                              activeTrade.symbol && debouncedSuggest(
                                activeTrade.symbol,
                                activeEditIndex,
                          ...
                          />
                          {symbolSuggestions.length > 0 && suggestForIdx === activeEditIndex && (
                            <SuggestionDropdown suggestions={symbolSuggestions} idx={activeEditIndex} />
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div>
                          <label className="block text-xs font-medium text-slate-300 mb-1.5">Lots</label>
                          <input
                            type="number"
                            className="w-full bg-slate-600 border border-slate-500 rounded-lg px-3 py-2.5 text-white placeholder-slate-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            ...
                            onChange={e => {
                              const arr = [...plannedTrades];
                              arr[activeEditIndex].lots = parseInt(e.target.value) || 1;
                              arr[activeEditIndex].quantity = calculateQuantity(arr[activeEditIndex]);
                              setPlannedTrades(arr);
                            }}
                          />
                        </div>
                        ...
                        <div>
                          <label className="block text-xs font-medium text-slate-300 mb-1.5">Entry Price</label>
                          <input
                            type="number"
                            step="0.01"
                            className="w-full bg-slate-600 border border-slate-500 rounded-lg px-3 py-2.5 text-white placeholder-slate-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            ...
                            onChange={e => {
                              const arr = [...plannedTrades];
                              arr[activeEditIndex].entry = parseFloat(e.target.value) || 0;
                              setPlannedTrades(arr);
                            }}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={e => {
                              const arr = [...plannedTrades];
                              arr[activeEditIndex].stopLoss = parseFloat(e.target.value) || 0;
                              setPlannedTrades(arr);
                            }}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={e => {
                              const arr = [...plannedTrades];
                              arr[activeEditIndex].target = parseFloat(e.target.value) || 0;
                              setPlannedTrades(arr);
                            }}
                          />
                        </div>
                        <div>
                          <label className="block text-xs font-medium text-slate-300 mb-1.5">Projected P&L</label>
                          <input
                            type="text"
                            className="w-full bg-slate-600 border border-slate-500 rounded-lg px-3 py-2.5 text-white placeholder-slate-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                      ...
                      <div>
                        <label className="block text-xs font-medium text-slate-300 mb-1.5">Trade Rationale</label>
                        <textarea
                      ...
                      {(activeTrade.instrumentName || activeTrade.segment || activeTrade.lotSize || activeTrade.expiry) && (
                        <div className="bg-slate-600/30 rounded-lg p-3 border border-slate-600">
                          <div className="text-[11px] font-medium text-slate-300 mb-1.5">Instrument Details</div>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                            {activeTrade.instrumentName && (
                              <div>
                                <span className="text-slate-400">Name:</span>
                                <div className="text-white font-medium">{activeTrade.instrumentName}</div>
                              </div>
                            )}
                            {activeTrade.segment && (
                              <div>
                                <span className="text-slate-400">Segment:</span>
                                <div className="text-white font-medium">{activeTrade.segment}</div>
                              </div>
                            )}
                            {activeTrade.lotSize && (
                              <div>
                                <span className="text-slate-400">Lot Size:</span>
                                <div className="text-white font-medium">{activeTrade.lotSize}</div>
                              </div>
                            )}
                            {activeTrade.expiry && (
                              <div>
                                <span className="text-slate-400">Expiry:</span>
                                <div className="text-white font-medium">{activeTrade.expiry}</div>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-slate-600">
                        <div className="text-slate-400 text-sm">
                          <span className="font-medium">Total Projected P&L: </span>
                          <span className={`text-lg font-bold ${approxPL >= 0 ? "text-green-400" : "text-red-400"}`}>
                  ...
                  {plannedTrades?.length > 0 && plannedTrades[0].symbol ? (
                    <>
                      {planNotes && (
                        <div>
                          ...
                          <div className="bg-slate-700/30 rounded-xl p-5 border border-slate-600">
                            <p className="text-slate-200 leading-relaxed">{planNotes}</p>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="bg-slate-700/30 rounded-xl p-5 border border-slate-600">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
                          <div className="text-center">
                            ...
                            <div className="text-2xl font-bold text-white">{psych.confidenceLevel}/10</div>
                          </div>
                          <div className="text-center">
                            <div className="text-xs text-slate-400 mb-1">Stress</div>
                            <div className="text-2xl font-bold text-white">{psych.stressLevel}/10</div>
                          </div>
                          <div className="text-center">
                            <div className="text-xs text-slate-400 mb-1">Sleep</div>
                            <div className="text-2xl font-bold text-white">{psych.sleepHours}h</div>
                          </div>
                          <div className="text-center">
                            <div className="text-xs text-slate-400 mb-1">Distractions</div>
                            <div className="text-lg font-medium text-white">{psych.distractions || "None"}</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        <h4 className="text-base font-semibold text-white mb-3 flex items-center gap-2">
                          <Target className="w-5 h-5 text-indigo-400" />
                          Planned Trades ({plannedTrades.filter(t => t.symbol).length})
                        </h4>
                        <div className="overflow-x-auto">
                          <table className="w-full border-collapse">
                            <thead>
                            ...
                            <tbody>
                              {plannedTrades.map((t, i) => {
                                ...
                                return (
                                  <tr key={i} className="border-b border-slate-700 hover:bg-slate-700/20 transition-colors">
                                    <td className="p-2">
                                      <div className="font-medium text-white">{t.symbol}</div>
                                      {t.expiry && <div className="text-[11px] text-slate-400">{t.expiry}</div>}
                                    </td>
                                    ...
                                    <td className="p-2 text-slate-300 text-xs text-center">{t.strategy}</td>
                                    <td className="p-2 text-center text-white font-medium">₹{t.entry || "0.00"}</td>
                                    <td className="p-2 text-center text-red-400 font-medium">₹{t.stopLoss || "0.00"}</td>
                                    <td className="p-2 text-center text-green-400 font-medium">₹{t.target || "0.00"}</td>
                                    <td className="p-2 text-center text-white">{t.lots}</td>
                                    ...
                                    <td className={`p-2 text-center font-medium ${
                                      potentialPL >= 0 ? "text-green-400" : "text-red-400"
                                    }`}>
                                      ₹{potentialPL.toFixed(2)}
                                    </td>
                        ...
                        {plannedTrades.some(t => t.reason) && (
                          <div className="mt-5">
                            <h5 className="text-xs font-semibold text-slate-400 mb-2">Trade Rationales</h5>
                                ...
                                t.reason && (
                                  ...
                                  >
                                    <div className="flex items-center gap-3 min-w-0">
                                      ...
                                      {t.expiry && (
                                        <span className="text-[11px] text-slate-400 ml-2">{t.expiry}</span>
                                      )}
                                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div className="bg-slate-700/30 rounded-lg p-4 border border-slate-600">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="text-slate-300 text-sm">Planned Trades</span>
                            </div>
                          ...
                          <div className="bg-slate-700/30 rounded-lg p-4 border border-slate-600">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="text-slate-300 text-sm">Projected P&L</span>
                            </div>
                            <div className={`text-xl font-bold ${approxPL >= 0 ? "text-green-400" : "text-red-400"}`}>
                              {approxPL >= 0 ? "+" : ""}₹{approxPL.toFixed(2)}
                            </div>
                          </div>
                          <div className="bg-slate-700/30 rounded-lg p-4 border border-slate-600">
                            ...
                            <div className="text-lg font-semibold text-amber-400">Moderate</div>
                          </div>
                        </div>
                      </div>
                    </>
                    ...
                    <div className="text-center py-6">
                      <div className="mb-5">
                        ...
                        <h3 className="text-lg font-semibold text-slate-400 mb-1">No Trading Plan</h3>
                        <p className="text-slate-500">Create your strategic trading plan for this date</p>
                      </div>
                      <button
